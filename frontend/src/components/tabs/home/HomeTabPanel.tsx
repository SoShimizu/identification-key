// frontend/src/components/tabs/home/HomeTabPanel.tsx
...
export default function HomeTabPanel({ lang }: Props) {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', flexShrink: 0, display: 'flex', justifyContent: 'center' }}>
        <Tabs value={tabIndex} onChange={handleTabChange} aria-label="home-sub-tabs">
          <Tab icon={<InfoIcon />} iconPosition="start" label={ja ? "概要" : "Overview"} />
          <Tab icon={<SpeedIcon />} iconPosition="start" label={ja ? "クイックスタート" : "Quick Start"} />
          <Tab icon={<ArticleIcon />} iconPosition="start" label={ja ? "マトリクス作成ガイド" : "Matrix Guide"} />
          <Tab icon={<GitHubIcon />} iconPosition="start" label={ja ? "このアプリについて" : "About & Citation"} />
        </Tabs>
      </Box>
      ...
      <Box sx={{ flex: 1, minHeight: 0, overflow: 'hidden' }}>
        <TabPanel value={tabIndex} index={0}>
            <Card variant="outlined">
                <CardHeader title={ja ? "MyKeyLogueとは？" : "What is MyKeyLogue?"} />
                <CardContent>
